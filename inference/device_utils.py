...
def normalize_runtime_device(device: str | None, default: str = "cuda") -> str:
...
def resolve_onnx_providers(device: str | None, *, label: str = "ONNX") -> tuple[str, list[ProviderSpec]]:
    ...
    if normalized == "cuda" and "CUDAExecutionProvider" in available:
        print(f"[{label}] Using CUDAExecutionProvider.")
        return "cuda", ["CUDAExecutionProvider", "CPUExecutionProvider"]
    if normalized == "cuda":
        print(f"[{label}] CUDAExecutionProvider unavailable; trying DirectML/CPU.")
