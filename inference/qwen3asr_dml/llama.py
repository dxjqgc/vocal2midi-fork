...
def init_llama_lib():
    ...
    # On Linux the compiled libs carry versioned SONAMEs (libggml.so.0,
    # libggml-base.so.0, libggml-cpu.so.0). Loading libggml.so by full path
    # still leaves the dynamic linker to resolve those NEEDED entries, and the
    # bin/ folder is not on the default search path -- so it fails with
    # "libggml-cpu.so.0: cannot open shared object file". Pre-load the
    # versioned files with RTLD_GLOBAL BEFORE the main libs so their symbols
    # are already visible when libggml.so's NEEDED is resolved, all without
    # touching LD_LIBRARY_PATH. Windows/macOS load by basename and are
    # unaffected.
    if sys.platform not in ("win32", "darwin"):
        for _soname in ("libggml-base.so.0", "libggml-cpu.so.0", "libggml.so.0"):
            _path = os.path.join(lib_dir, _soname)
            if os.path.exists(_path):
                try:
                    ctypes.CDLL(_path, mode=ctypes.RTLD_GLOBAL)
                except OSError:
                    # Best-effort: a missing versioned file falls back to the
                    # default search path for the main loads below.
                    pass
