...
class QwenAudioEncoder:
    ...
    def __init__(
        ...
        # use_dml=True means "use a GPU". On Linux there is no
        # DmlExecutionProvider, so prefer CUDA when available; resolve_onnx_providers
        # falls back to DML (Windows) / CPU automatically when CUDA is unavailable.
        requested_device = "cuda" if use_dml else "cpu"
